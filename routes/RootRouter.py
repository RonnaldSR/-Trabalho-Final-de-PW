...
async def post_alterar_senha(
    ...
    if not conferir_senha(usuario.senha, senha_atual):
        return templates.TemplateResponse(
            "root/restrito.html",
            {"request": request, "usuario": usuario, "erro_senha": "Senha atual incorreta."},
        )

    if nova_senha != conf_nova_senha:
        return templates.TemplateResponse(
            "root/restrito.html",
            {"request": request, "usuario": usuario, "erro_senha": "As novas senhas não coincidem."},
        )

    usuario.senha = obter_hash_senha(nova_senha)
    UsuarioRepo.atualizar_senha(usuario)

    return templates.TemplateResponse(
        "root/restrito.html",
        {"request": request, "usuario": usuario, "mensagem_senha": "Senha atualizada com sucesso."},
    )
